...
選單第 8 項「設定上下文 100 萬／最大輸出 128000」也可用
...
請先以最新安裝器執行 `update`，再套用這個設定。
功能會備份設定、將全域 `model_context_window` 設為 1000000，並將全部模型目錄的
上下文與最大輸出設為 1000000／128000。保留 95% 安全餘量，重建目錄時沿用設定。
